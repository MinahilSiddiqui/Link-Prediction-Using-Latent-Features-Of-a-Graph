...
def combined(g,pos_edges,neg_edges):
    ...
    for edge in pos_edges + neg_edges:
        ...
        aa = 0
        for i in neighbors_one.intersection(neighbors_two):
            neighbors_i = list(g.neighbors(i))
            if math.log(len(neighbors_i)) == 0:
                aa=0
            else:
                aa += 1 / math.log(len(neighbors_i))
