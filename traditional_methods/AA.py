...
def adamic_adar(g, pos_edges, neg_edges):
    ...
    for edge in pos_edges + neg_edges:
        ...
        result1.append((aa,label))
    with open("AAtwt.csv", 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)  
        writer.writerow(['adamic_adar', 'label'])
        writer.writerows(result1)

feature_set = [ adamic_adar ]
...
def sample_extraction(g, pos_num, neg_num, delete=1):
    ...
    # compute adamic adar for all samples
    adamic_adar(g,pos_sample,list(neg_sample))
...
r=np.loadtxt(open("AAtwt.csv", "rb"), delimiter=",", skiprows=1);
l,b=r.shape;
