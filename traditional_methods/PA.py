...
def preferential_attachment(g, pos_edges, neg_edges):
    result1=[]
    for edge in pos_edges + neg_edges:
        node_one, node_two = edge[0], edge[1]
        degree_one = g.degree(node_one)
        degree_two = g.degree(node_two)
        pref_attachment = degree_one * degree_two
        label = 1 if edge in pos_edges else 0
        result1.append((pref_attachment, label))
    with open("patwt.csv", 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)  
        writer.writerow(['pref_attachment', 'label'])
        writer.writerows(result1)
feature_set = [preferential_attachment ]
...
def sample_extraction(g, pos_num, neg_num, delete=1):
    ...
    # compute prefential attachment for all samples
    preferential_attachment(g,pos_sample,list(neg_sample))
...
r=np.loadtxt(open("patwt.csv", "rb"), delimiter=",", skiprows=1);
l,b=r.shape;
np.random.shuffle(r);
train_l=int(0.7*l)
