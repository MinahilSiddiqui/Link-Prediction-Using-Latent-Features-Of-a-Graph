...
def resource_allocation(g, pos_edges, neg_edges):
    result1=[]
    for edge in pos_edges + neg_edges:
        ...
        result1.append((ra,label))
    with open("ratwt.csv", 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)  
        writer.writerow(['resource_allocation', 'label'])
        writer.writerows(result1)
feature_set = [ resource_allocation ]
...
def sample_extraction(g, pos_num, neg_num, delete=1):
    ...
    # compute resource allocation for all samples
    resource_allocation(g,pos_sample,list(neg_sample))
...
r=np.loadtxt(open("ratwt.csv", "rb"), delimiter=",", skiprows=1);
l,b=r.shape;
np.random.shuffle(r);
train_l=int(0.7*l)
