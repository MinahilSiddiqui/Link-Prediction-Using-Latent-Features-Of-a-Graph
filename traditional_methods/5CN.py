...
def common_5hop_neighbors(g, pos_edges, neg_edges):
    result = []
    result1 = []
    for edge in pos_edges + neg_edges:
        ...
        neighbors_one_3hop = set()
        for neighbor in neighbors_one_2hop:
            try:
                neighbors_one_3hop.update(g.neighbors(neighbor))
            except:
                pass
                
        neighbors_two_3hop = set()
        for neighbor in neighbors_two_2hop:
            try:
                neighbors_two_3hop.update(g.neighbors(neighbor))
            except:
                pass
        
        neighbors_one_4hop = set()
        for neighbor in neighbors_one_3hop:
            try:
                neighbors_one_4hop.update(g.neighbors(neighbor))
            except:
                pass
                
        neighbors_two_4hop = set()
        for neighbor in neighbors_two_3hop:
            try:
                neighbors_two_4hop.update(g.neighbors(neighbor))
            except:
                pass
                
        neighbors_one_5hop = set()
        for neighbor in neighbors_one_4hop:
            try:
                neighbors_one_5hop.update(g.neighbors(neighbor))
            except:
                pass
                
        neighbors_two_5hop = set()
        for neighbor in neighbors_two_4hop:
            try:
                neighbors_two_5hop.update(g.neighbors(neighbor))
            except:
                pass
        
        num_common_5hop_neighbors = len(neighbors_one_5hop.intersection(neighbors_two_5hop))
        label = 1 if edge in pos_edges else 0
        
        result.append((node_one, node_two, num_common_5hop_neighbors, label))
        result1.append((num_common_5hop_neighbors,label))
    
    with open("output_file.csv", 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(['node_one', 'node_two', 'num_common_5hop_neighbors', 'label'])
        writer.writerows(result)
    
    with open("features.csv", 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)  
        writer.writerow(['num_common_5hop_neighbors', 'label'])
...
def sample_extraction(g, pos_num, neg_num, delete=1):
    ...
    common_5hop_neighbors(g,pos_sample,list(neg_sample))
    return pos_sample, neg_sample

feature_set = [common_5hop_neighbors]
