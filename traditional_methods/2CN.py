...
def common_2hop_neighbors(g, pos_edges, neg_edges):
    result1 = []
    for edge in pos_edges + neg_edges:
        ...
        num_common_2hop_neighbors = len(neighbors_one.intersection(neighbors_two))
        label = 1 if edge in pos_edges else 0
        
        result1.append((num_common_2hop_neighbors,label))
    with open("2cntwt.csv", 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)  
        writer.writerow(['num_common_2hop_neighbors', 'label'])
...
def sample_extraction(g, pos_num, neg_num, delete=1):
    ...
    common_2hop_neighbors(g,pos_sample,list(neg_sample))
    return pos_sample, neg_sample

feature_set = [ common_2hop_neighbors ]
...
#giving data to ML models
r=np.loadtxt(open("2cntwt.csv", "rb"), delimiter=",", skiprows=1);
l,b=r.shape;
np.random.shuffle(r);
train_l=int(0.7*l)
