...
def common_4hop_neighbors(g, pos_edges, neg_edges):
    result = []
    result1 = []
    for edge in pos_edges + neg_edges:
        ...
        num_common_4hop_neighbors = len(neighbors_one_3hop.intersection(neighbors_two_3hop))
        label = 1 if edge in pos_edges else 0
        
        result.append((node_one, node_two, num_common_4hop_neighbors, label))
        result1.append((num_common_4hop_neighbors,label))
    
    with open("output_file.csv", 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(['node_one', 'node_two', 'num_common_4hop_neighbors', 'label'])
        writer.writerows(result)
    
    with open("features.csv", 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)  
        writer.writerow(['num_common_4hop_neighbors', 'label'])
...
def sample_extraction(g, pos_num, neg_num, delete=1):
    ...
    common_4hop_neighbors(g,pos_sample,list(neg_sample))
    return pos_sample, neg_sample


feature_set = [ common_4hop_neighbors ]
def main(filename="facebook_combined.txt",model="combined",feature_name=feature_set):
...
main(filename=fn,model="combined", feature_name=feature_set)
