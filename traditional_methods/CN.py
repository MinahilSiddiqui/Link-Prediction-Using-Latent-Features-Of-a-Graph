...
def common_neighbors(g, pos_edges, neg_edges):
    result1=[]
    for edge in pos_edges + neg_edges:
        ...
        result1.append((num_common_neighbors,label))
    with open("cntwitter.csv", 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)  
        writer.writerow(['num_common_neighbors', 'label'])
...
def sample_extraction(g, pos_num, neg_num):
    ...
    common_neighbors(g,pos_sample,list(neg_sample))
...
r=np.loadtxt(open("cntwitter.csv", "rb"), delimiter=",", skiprows=1);
l,b=r.shape;
np.random.shuffle(r);
train_l=int(0.7*l)
